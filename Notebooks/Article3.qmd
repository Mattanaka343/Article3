...
```
y nos permite generar la función de distribución global condicional $$f(y,x_1,x_2,\ldots, x_n) = f(y)\prod_{i=1}^nf_i(x_i\mid y)$$ Este tipo de red tamién nos permite clasificar usando la regla de bayes en la cuál para una nueva observación $\tilde{x}$ la probabilidad de $$\mathbb{P}(Y=k \mid X=\tilde{x}) = \frac{\mathbb{P}(X = \tilde{x} \mid Y=k)\mathbb{P}(Y=k)}{\mathbb{P}(X=\tilde{x})}$$ donde $\mathbb{P}(x) = \sum_{1}^k \mathbb{P}(X= \tilde{x} \mid Y= k)\mathbb{P}(Y=k)$. La clase asignada a cada observación es aquella con la mayor probabilidad a posteriori

Este proyecto también utilizó una técnica conocida como *web scrapping*, esta se refiere a recolectar datos de algún sitio web extrayendolos de su código fuente (usualmente html). Aunado a esto se utilizó una técnica de *tokenización* (asignación de valores numéricos a plabras) conocida como *count vectorizer* que consiste en transformar el texto en una matriz donde cada palabra única se convierte en una columna y cada documento en una fila. El valor numérico asignado a cada palabra corresponde a la cantidad de veces que aparece en ese documento. Finalmente se usaron técnicas de eliminación de *stop words* (palabras que no añaden información) para mantener únicamente las palabras que añaden al texto.
