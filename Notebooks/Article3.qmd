...
Con los textos analizados se utilizó el moodelo de clasifación conocido como *Naïve Bayes* para categorizar historias nuevas en alguno de los eventos que incluímos (posesiones exorcismos y demonios o poltergeists y manifestaciones físicas). Para entrenar este modelo se dividieron los datos en 80% para entranmiento y 20% para pruebas. 
```{R}
spooky_matrix = words |>
  anti_join(stop_words) |>
...
spooky_split = initial_split(data, prop = 0.8)

training_data = training(spooky_split)
testing_data = testing(spooky_split)

classifier = naive_bayes(type_of_story ~ ., data = training_data, laplace = TRUE)
