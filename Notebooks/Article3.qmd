...
Investigaciones previas [@bohrn2025neural]han mostrado que existen diferencias cognitivas reales en cómo los lectores procesan las narrativas con elementos sobrenaturales frente a las realistas. En la primera instancia se se ve activado mayormente el córtex sensoriomotor, el cual se encarga del aspecto de simulaciones motoras y perceptuales, por otro lado, el giro frontal inferior recibió mayor estimulación al momento de leer relatos con personajes realistas. Lo que esto indica es una diferencia respecto al uso del lenguaje, [@elsayed2021language]a través del empleo de recursos literarios, como son metáforas relacionadas con alguna transformación física, referencias a la posesión (objetos o seres vivos), y en ciertos casos violencia, por tal motivo, la creación de un clasificador resulta pertinente y adecuado dado que sí se presentan variaciones reales en la literatura. En las siguientes secciones se presentan y describen los métodos utilizados, los resultados obtenidos y su interpretación en el entorno literario.
...
Finalmente se anlaizó cómo las palabras determinan los sentimientoos implicados por una historia. Para esto se le asignó un valor numérico a cada una de las palabras representando su connotación emocional. El tono de cada historia fue considerado como la suma de los valores sentimentales de sus palabras. Este análisis mostró que la mayoría de las historias son significativamente más negativas que positivas con solo 20 títulos siendo mayormente positivos o neutros. Debido a la materia de las historias que se analizaron, resultados de este índole eran de esperarse, las historias de terror tienden a ser bastante negativas y esto se exalta cuando se habla de posesiones, demonios, exorcismos, poltergeists y manifestaciones físicas.
...
Con los textos analizados se utilizó el moodelo de clasifación conocido como *Naïve Bayes* para categorizar historias nuevas en alguno de los eventos que incluimos (posesiones exorcismos y demonios o poltergeists y manifestaciones físicas). Para entrenar este modelo se dividieron los datos en 80% para entrenamiento y 20% para pruebas. Este modelo resultó en la siguiente matriz de confusión
```{R}
set.seed(42)
...
metrics
```
El modelo presenta una exactitud (*accuracy*) de 85% siendo bastante bueno. Presenta una exhaustividad (*recall*) de 80% y 90% respectivamente lo cual evoca que no predice ninguna clase desproporcionadamente. Finalmente presenta precisiones de 89% y 82% por lo que se puede inferir que el valor de predicciones positivas se acercó mucho al real sustentando la aseveración de que no sobrepredice ninguna clase. 

## Conclusión
Para finalizar este artículo se muestran los resultados obtenidos a lo largo del trabajo. El modelo utilizado fue capaz de puntuar bastante alto debido a que consiguió acertar 34 predicciones respecto a 40 historias, lo que equivale a una exactitud del 85%, un porcentaje considerablemente bueno. Asimismo obtuvo una exhaustividad del 80% y 90% en sus clases, de esto se infiere que ninguna clase engulle a la otra al momento de las predicciones. En última instancia las precisiones obtenidas confirman todos los valores previamente mencionados, puesto que se logró un 89% y 92% de precisión. Alcanzar estos puntajes permite aclarar que el modelo predice bastante bien respecto a los relatos a los cuales fue expuesto. A su vez, la importancia de estos resultados radica en conseguir mejores y más acertados modelos que sean capaces de predecir casi a la perfección no solo para clasificación de textos, sino para cualquier ámbito o campo que se desee, y así de esta manera, mejorar la experiencia del usuario, eficientizar procesos y minimizar errores de esta índole. La sugerencia dada podría ser una mayor recopilación de textos, al igual que una investigación más extensa y profunda acudiendo a expertos en el área, como escritores, neurolingüistas, filólogos y/o psicólogos, del mismo modo estudiar la evolución de la narrativa y comparar cómo los distintos contextos socioculturales afectan al lenguaje podría ser útil si se desea replicar o simular este trabajo.
